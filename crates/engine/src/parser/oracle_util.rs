...
/// MTGJSON-derived subtype vocabulary (`crates/engine/data/oracle-subtypes.json`).
/// Regenerated by `oracle_gen` whenever card-data is rebuilt.
static ORACLE_SUBTYPES: std::sync::LazyLock<Vec<String>> = std::sync::LazyLock::new(|| {
    ...
    // Longest-first so multi-word subtypes ("Time Lord") win over prefixes ("Time").
